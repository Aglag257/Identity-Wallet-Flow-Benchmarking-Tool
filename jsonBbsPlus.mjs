...
/*  Global issuer key (BBS+)  */
const issuerBbs = await generateBls12381G2KeyPair();
...
async function startIssuer(port=ISSUER_PORT){
  ...
  app.post("/credential", async (req,res)=>{
    ...
    try{
      ...
      // if(!proofs?.attestation?.length) return res.status(400).json({error:"missing_proofs"});
      ...
      const signature = await blsSign({ keyPair: issuerBbs, messages });
